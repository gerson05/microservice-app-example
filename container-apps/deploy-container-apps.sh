...
# This script deploys microservices to Azure Container Apps using the correct approach
...
deploy_app "zipkin-app" \
    ...
    "STORAGE_TYPE=elasticsearch ES_HOSTS=https://elasticsearch-app.internal:9200"

# 3. Log Processor (depends on Redis and Zipkin)
deploy_app "log-processor-app" \
    "$ACR_LOGIN_SERVER/log-message-processor:latest" \
    "null" "internal" "0.1" "256Mi" "1" "2" \
    "REDIS_HOST=microservices-redis.redis.cache.windows.net REDIS_PORT=6380 REDIS_CHANNEL=log_channel ZIPKIN_URL=https://zipkin-app.internal/api/v2/spans"

# 4. Users API (depends on Redis and Zipkin)
deploy_app "users-api-app" \
    "$ACR_LOGIN_SERVER/users-api:latest" \
    "8083" "internal" "0.5" "1Gi" "1" "3" \
    "SERVER_PORT=8083 JWT_SECRET=myfancysecret SPRING_ZIPKIN_BASEURL=https://zipkin-app.internal/ SPRING_PROFILES_ACTIVE=production SPRING_REDIS_HOST=microservices-redis.redis.cache.windows.net SPRING_REDIS_PORT=6380"

# 5. Auth API (depends on Users API and Zipkin)
deploy_app "auth-api-app" \
    "$ACR_LOGIN_SERVER/auth-api:latest" \
    "8081" "internal" "0.25" "512Mi" "1" "3" \
    "AUTH_API_PORT=8081 JWT_SECRET=myfancysecret USERS_API_ADDRESS=https://users-api-app.internal ZIPKIN_URL=https://zipkin-app.internal/api/v2/spans"

# 6. Todos API (depends on Redis and Zipkin)
deploy_app "todos-api-app" \
    "$ACR_LOGIN_SERVER/todos-api:latest" \
    "8082" "internal" "0.25" "512Mi" "1" "5" \
    "TODO_API_PORT=8082 JWT_SECRET=myfancysecret REDIS_HOST=microservices-redis.redis.cache.windows.net REDIS_PORT=6380 REDIS_CHANNEL=log_channel ZIPKIN_URL=https://zipkin-app.internal/api/v2/spans"

# 7. Frontend (depends on Auth API and Todos API)
deploy_app "frontend-app" \
    "$ACR_LOGIN_SERVER/frontend:latest" \
    "8080" "external" "0.5" "1Gi" "1" "3" \
    "NODE_ENV=production AUTH_API_ADDRESS=https://auth-api-app.internal TODOS_API_ADDRESS=https://todos-api-app.internal ZIPKIN_URL=https://zipkin-app.internal/api/v2/spans"
...
echo ""
echo "🧹 To clean up:"
echo "  az containerapp list --resource-group $RESOURCE_GROUP --query '[].name' --output tsv | xargs -I {} az containerapp delete --name {} --resource-group $RESOURCE_GROUP --yes"
